...
def comparisonPlot(logname, dirs, folder, warm_start, mean_size=0, max = -1):
	...
	for i in range(0, len(dirs)):
		dataframe = pd.read_csv("results/" + dirs[i] + "/logs/" + logname + "_log.txt", header=None, index_col=False) # create a dataframe
...
if __name__ == '__main__':
	"""
	Main method called separatly from the machine learning architecture to create plots of gathered result data after training experiments.
	dirname should be set to match the result folder of the given data.
	"""
	# createPlots(100, 500)
